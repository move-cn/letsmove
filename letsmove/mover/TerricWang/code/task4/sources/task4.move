module task4::terric_game {
    ...
    fun get_spin_result(r: &Random, ctx: &mut TxContext) : SpinResult {
        let mut generator = new_generator(r, ctx);
        let spin1 = (random::generate_u64_in_range(&mut generator, 0, 3) as u8);
        SpinResult { spin1 }
    }
    ...
    public entry fun play<T>(game: &mut Game<T>, input: Coin<T>, r: &Random, ctx: &mut TxContext) {
        ...
        let player_spins = get_spin_result(r, ctx);
        let winning_combination = SpinResult { spin1: 2 }; 
        ...
        let (result, is_winner) = if (player_spins.spin1 == winning_combination.spin1 ) {
