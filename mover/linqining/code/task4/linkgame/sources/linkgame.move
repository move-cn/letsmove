...
public struct CoinGame  has key,store{
    id: UID,
    // 存钱必须用这个结构体，
    amount: Balance<EIG>,
}
...
entry fun play(game: &mut CoinGame, rand:&Random, guess_val:bool, user_bet_coin: coin::Coin<EIG>, ctx: &mut TxContext){
...
public entry fun deposit(game:&mut CoinGame,amount: coin::Coin<EIG>,_:&mut TxContext){
    let in_amt = coin::into_balance(amount);
    game.amount.join(in_amt);
}

public entry fun withdraw(_:&AdminCap,game:&mut CoinGame,amount: u64,ctx:&mut TxContext){
