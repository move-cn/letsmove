...
module task4::lbiyou_game {
 ...
 use std::string::{Self, String};
    ...
    fun map_choice_to_string(choice: u8) : String {
        if (choice == 0) {
            string::utf8(b"lbiyou_rock")
        } else if (choice == 1) {
            string::utf8(b"lbiyou_paper")
        } else if (choice == 2) {
            string::utf8(b"lbiyou_scissors")
        } else {
    ...
    public entry fun play(choice: u8, clock: &Clock) {
        ...
        let (result, is_win) = if (
            ...
            (string::utf8(b"you Win lbiyou"), true)
        } else if (choice == computer_choice) {
            (string::utf8(b"you Draw lbiyou"), false)
        } else {
            (string::utf8(b"you Lose lbiyou"), false)
        };
