module task4::game {
    ...
    use task2::giizhang_faucet_coin::{Self, GIIZHANG_FAUCET_COIN};
    ...
    public struct Game has key {
        id: UID,
        pool: Balance<GIIZHANG_FAUCET_COIN>,
        ticket: u64,
        reward: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<GIIZHANG_FAUCET_COIN>(),
            ticket: 1_000,
            reward: 2_000,
        };
    ...
    public entry fun play(game: &mut Game, input: Coin<GIIZHANG_FAUCET_COIN>, r: &Random, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<GIIZHANG_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<GIIZHANG_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        giizhang_faucet_coin::mint(treasury_cap, amount, sender(ctx), ctx);
    }
}
