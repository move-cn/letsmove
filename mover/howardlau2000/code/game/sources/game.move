module game::howardlau2000_game {
    ...
    use coin::howardlau2000_faucet_coin::{Self, HOWARDLAU2000_FAUCET_COIN};
    ...
    public struct Game has key {
        id: UID,
        pool: Balance<HOWARDLAU2000_FAUCET_COIN>,
        ticket: u64,
        reward: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<HOWARDLAU2000_FAUCET_COIN>(),
            ticket: 1_000,
            reward: 2_000,
        };
    ...
    public entry fun play(game: &mut Game, input: Coin<HOWARDLAU2000_FAUCET_COIN>, clock: &Clock, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<HOWARDLAU2000_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<HOWARDLAU2000_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        howardlau2000_faucet_coin::mint(treasury_cap, amount, sender(ctx), ctx);
    }
}
