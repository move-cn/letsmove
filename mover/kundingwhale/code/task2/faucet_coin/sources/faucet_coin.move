module faucet_coin::kundingwhale {
    use std::option;
    use sui::object::{UID};
    ...
    struct Wallet has key {
        id: UID,
        coin: Balance<KUNDINGWHALE>
    }

    fun init(witness: KUNDINGWHALE, ctx: &mut TxContext) {
        let (treasury, metadata) = coin::create_currency<KUNDINGWHALE>(witness, 6, b"WOOD", b"", b"", option::none(), ctx);
    ...
    public entry fun mint(
        treasury_cap: &mut TreasuryCap<KUNDINGWHALE>, wallet: &mut Wallet, amount: u64, ctx: &mut TxContext
    ) {
