...
//用户玩游戏，赢了双倍奖励
public entry fun play(userCoin: &mut Coin<FAUCET_YJ9957>, amount: u64, pool: &mut GamePool, in: bool, rand: &Random, ctx: &mut TxContext) {
...
//往奖池充入余额
public entry fun save_coin(_: &AdminCap, userCoin: &mut Coin<FAUCET_YJ9957>, amount: u64, pool: &mut GamePool, ctx: &mut TxContext) {
    let splitCoin =  userCoin.split(amount, ctx);
    let splitBalance = coin::into_balance(splitCoin);
    pool.amount.join(splitBalance);
}
