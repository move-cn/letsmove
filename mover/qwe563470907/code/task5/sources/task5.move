...
public struct Swap has key{
    id: UID,
    coin: Balance<QWE>,
    faucet_coin: Balance<FAUCET_QWE>
}
...
fun init(ctx: &mut TxContext) {
    ...
    let swap = Swap{
        id: object::new(ctx),
        coin: balance::zero<QWE>(),
        faucet_coin: balance::zero<FAUCET_QWE>()
    };
...
public entry fun swap_coin_to_faucet(swap: &mut Swap, userCoin: &mut Coin<QWE>, userCoinAmount: u64, ctx: &mut TxContext) {
...
public entry fun swap_faucet_to_coin(swap: &mut Swap, userFaucetCoin: &mut Coin<FAUCET_QWE>, userFaucetCoinAmount: u64, ctx: &mut TxContext) {
...
public entry fun save_coin(_: &AdminCap, swap: &mut Swap, userCoin: &mut Coin<QWE>, userCoinAmount: u64, userFaucetCoin: &mut Coin<FAUCET_QWE>, userFaucetCoinAmount: u64, ctx: &mut TxContext) {
    if (userCoinAmount > 0) {
