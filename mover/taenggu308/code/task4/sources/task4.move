module task4::game {
    ...
    use task2::taenggu308_faucet_coin::{Self, TAENGGU308_FAUCET_COIN};
    ...
    public struct Game has key {
        id: UID,
        pool: Balance<TAENGGU308_FAUCET_COIN>,
        ticket: u64,
        reward: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<TAENGGU308_FAUCET_COIN>(),
            ticket: 1_000,
            reward: 2_000,
        };
    ...
    public entry fun play(game: &mut Game, input: Coin<TAENGGU308_FAUCET_COIN>, r: &Random, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<TAENGGU308_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<TAENGGU308_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        taenggu308_faucet_coin::mint(treasury_cap, amount, sender(ctx), ctx);
    }
}
