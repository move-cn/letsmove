module lets_move::lets_move {
    ...
    entry fun get_flag(
        ...
        vector::append<u8>(&mut full_proof, proof); // 随机数
        vector::append<u8>(&mut full_proof, tx_context::sender(ctx).to_bytes()); // 发送者
        vector::append<u8>(&mut full_proof, bcs::to_bytes(challenge)); // challenge字符串

        let hash: vector<u8> = hash::sha3_256(full_proof);

        // challenge.difficulity = 3 前三位为0
