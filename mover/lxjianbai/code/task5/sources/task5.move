...
use task2::faucet_lxjianbai::FAUCET_LXJIANBAI;
use task2::lxjianbai::LXJIANBAI;
...
public struct Swap has key{
    id: UID,
    coin: Balance<LXJIANBAI>,
    faucet_coin: Balance<FAUCET_LXJIANBAI>
}
...
fun init(ctx: &mut TxContext) {
    ...
    let swap = Swap{
        id: object::new(ctx),
        coin: balance::zero<LXJIANBAI>(),
        faucet_coin: balance::zero<FAUCET_LXJIANBAI>()
    };
...
public entry fun swap_coin_to_faucet(swap: &mut Swap, userCoin: &mut Coin<LXJIANBAI>, userCoinAmount: u64, ctx: &mut TxContext) {
...
public entry fun swap_faucet_to_coin(swap: &mut Swap, userFaucetCoin: &mut Coin<FAUCET_LXJIANBAI>, userFaucetCoinAmount: u64, ctx: &mut TxContext) {
...
public entry fun save_coin(_: &AdminCap, swap: &mut Swap, userCoin: &mut Coin<LXJIANBAI>, userCoinAmount: u64, userFaucetCoin: &mut Coin<FAUCET_LXJIANBAI>, userFaucetCoinAmount: u64, ctx: &mut TxContext) {
    if (userCoinAmount > 0) {
