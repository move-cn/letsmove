...
module move_game::flip_coin {
    ...
    entry fun play(game: &mut Game, flip_value: bool, in: Coin<M2887_FAUCET_COIN>, rand: &Random,
                   ctx: &mut TxContext) {
    ...
    // 存钱
    public entry fun add_sui(game: &mut Game, in: Coin<M2887_FAUCET_COIN>, _ctx: &TxContext) {
        let in_balance = into_balance(in);
        balance::join(&mut game.val, in_balance);
    }
