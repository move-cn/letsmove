/// Module: capy_faucet
module capy_faucet::capy_faucet {
    use sui::balance::{Balance};
    use sui::balance;
    use sui::coin::{Self, TreasuryCap};
    public struct CAPY_FAUCET has drop {}
    public struct PublicWallet has key {
        id: UID,
        coin: Balance<CAPY_FAUCET>,
        faucet_amount: u64,
    }
    const AMOUNT: u64 = 10^12;
    const EFaucetDry: u64 = 1;
    #[allow(lint(share_owned))]
    fun init(witness: CAPY_FAUCET, ctx: &mut TxContext) {
        let (treasury_cap, metadata) = coin::create_currency<CAPY_FAUCET>(
            witness,
            10,
            b"CAPY Faucet",
            b"CAPY Faucet",
    ...
    public entry fun mint_faucet(
        treasury_cap: &mut TreasuryCap<CAPY_FAUCET>,
        amount: u64,
