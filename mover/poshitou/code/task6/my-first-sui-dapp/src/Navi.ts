...
import {nUSDC} from "navi-sdk/src/address.ts";
...
export const mainAction = async (account: any, { signAndExecute, usdcAmount, onSuccess = () => { } }: {
    ...
    const nUSDCPool: PoolConfig = pool[nUSDC.symbol as keyof Pool];

    if (!SuiPool || !nUSDCPool) {
    ...
    try {
        ...
        // 借出 usdc
        const [usdc_coin] = await borrowCoin(tx as any, nUSDCPool, usdcAmount);
        console.log("Borrowed USDC:", usdc_coin);

        // 存入 usdc
        await depositCoin(tx as any, nUSDCPool, usdc_coin, usdcAmount);
