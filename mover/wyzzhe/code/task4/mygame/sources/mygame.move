module mygame::wyzzhe_dice_game {
    ...
    use faucetcoin::wyzzheFaucet::{Self, WYZZHEFAUCET};
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<WYZZHEFAUCET>(),
            ticket: 1_000,
            reward: 2_000,
        };
    ...
    public entry fun play(choice: u8, game: &mut Game, input: Coin<WYZZHEFAUCET>, clock: &Clock, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<WYZZHEFAUCET>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<WYZZHEFAUCET>, amount: u64, ctx: &mut TxContext) {
        wyzzheFaucet::mint(treasury_cap, amount, sender(ctx), ctx);
    }
}