...
module move_nft::move_nft {
    ...
    public struct IRurlNFT has key , store {
        id: UID,
        name: String,
        traits: String,
        url: Url,
    ...
    public struct IRurlMinted has copy, drop {
        IRurl_id: ID,
    ...
    fun init(witness: MOVE_NFT, ctx: &mut TxContext){
        ...
        let mut _display = display::new<IRurlNFT>(&_publisher,ctx);
    ...
    public entry fun mint (
        ...
        event::emit(IRurlMinted {
            IRurl_id: object::uid_to_inner(&id),
    ...
    public entry fun transfer(
        nft: IRurlNFT, recipient: address, _: &mut TxContext
    ) {
        transfer::public_transfer(nft, recipient)
    }

    public fun name(nft: &IRurlNFT): &String {
        &nft.name
    }

    public fun traits(nft: &IRurlNFT): &String {
        &nft.traits
    }

    public fun url(nft: &IRurlNFT): &Url {
        &nft.url
    }
