...
module check_in::check_in_tests {
    ...
    #[test]
        ...
        ts::next_tx(&mut scenario, admin);
        {
            ...
            vector::append<u8>(&mut my_vector, check_in::get_vector(string(b"isQyoung"), &mut flag_str, scenario.ctx()));

            let len = vector::length(&my_vector);
            debug::print(&my_vector);
            let mut i = 0;
            while (i < len)
            {
        ...
        ts::next_tx(&mut scenario, admin);
        {
            ...
            check_in::get_flag(my_vector, string(b"isQyoung"), &mut flag_str, &rand, scenario.ctx());
