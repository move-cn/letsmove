module swap::swap {
    ...
    use coin_faucet::linktimes_faucet_coin::{LINKTIMES_FAUCET_COIN};
    use coin_faucet::linktimes_coin::{LINKTIMES_COIN};

    public struct Bank has key {
        id: UID,
        USD: Balance<LINKTIMES_COIN>,
        RMB: Balance<LINKTIMES_FAUCET_COIN>,
    ...
    fun init(ctx: &mut TxContext) {
        let bank = Bank {
            id: object::new(ctx),
            USD: balance::zero<LINKTIMES_COIN>(),
            RMB: balance::zero<LINKTIMES_FAUCET_COIN>(),
            rate: 7,
        };
    ...
    public entry fun deposit_RMB(bank:&mut Bank,rmb:Coin<LINKTIMES_FAUCET_COIN>,_:&mut TxContext){
        balance::join(&mut bank.RMB,coin::into_balance(rmb));
    }

    public entry fun deposit_USD(bank:&mut Bank,usd:Coin<LINKTIMES_COIN>,_:&mut TxContext){
        balance::join(&mut bank.USD,coin::into_balance(usd));
    }

    public entry fun swap_usd2rmb(bank: &mut Bank, usd: Coin<LINKTIMES_COIN>, ctx: &mut TxContext) {
    ...
    public entry fun swap_rmb2usd(bank: &mut Bank, rmb: Coin<LINKTIMES_FAUCET_COIN>, ctx: &mut TxContext) {
