module game::fyj1230_game {
    ...
    use coin::fyj1230_faucet_coin::{Self, FYJ1230_FAUCET_COIN};
    use std::hash;
    use std::vector;
    ...
    public struct Game has key {
        id: UID,
        pool: Balance<FYJ1230_FAUCET_COIN>,
        ticket: u64,
        reward: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<FYJ1230_FAUCET_COIN>(),
            ticket: 1_000,
            reward: 2_000,
        };
    ...
    public entry fun play(game: &mut Game, input: Coin<FYJ1230_FAUCET_COIN>, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<FYJ1230_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<FYJ1230_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        fyj1230_faucet_coin::mint(treasury_cap, amount, sender(ctx), ctx);
    }
