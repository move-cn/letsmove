/// Module: task4
module task4::halsey929_dice_game {
    ...
    use task2::halsey929_faucet_coin::{Self, HALSEY929_FAUCET_COIN};
    ...
    public struct Game has key {
        id: UID,
        pool: Balance<HALSEY929_FAUCET_COIN>,
        ticket: u64,
        reward: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<HALSEY929_FAUCET_COIN>(),
            ticket: 1_000,
            reward: 2_000,
        };
    ...
    public entry fun play(choice: u8, game: &mut Game, input: Coin<HALSEY929_FAUCET_COIN>, clock: &Clock, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<HALSEY929_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<HALSEY929_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        halsey929_faucet_coin::mint(treasury_cap, amount, sender(ctx), ctx);
    }
}
