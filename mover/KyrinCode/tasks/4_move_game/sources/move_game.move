module move_game::kyrincode_game {
    ...
    public struct Game has key {
        id: UID,
        pool: Balance<KYRINCODE_FAUCET_COIN>,
        ticket: u64,
        reward: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<KYRINCODE_FAUCET_COIN>(),
            ticket: 100_000_000,
            reward: 200_000_000,
        };
    ...
    public entry fun play(choice: u8, game: &mut Game, input: Coin<KYRINCODE_FAUCET_COIN>, clock: &Clock, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<KYRINCODE_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<KYRINCODE_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        kyrincode_faucet_coin::mint(treasury_cap, amount, sender(ctx), ctx);
    }
}
