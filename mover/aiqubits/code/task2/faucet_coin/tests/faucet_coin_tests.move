...
module faucet_coin::my_coin_tests {
    use faucet_coin::aiqubit_faucet::{Self, AIQUBIT_FAUCET, Wallet};
    ...
    fun mint_burn() {
        ...
        next_tx(&mut scenario, addr1);
        {
            ...
            let mut wallet = aiqubit_faucet::new_wallet(test_scenario::ctx(&mut scenario), balance::zero());

            let mut treasury_cap = test_scenario::take_from_sender<TreasuryCap<AIQUBIT_FAUCET>>(&scenario);

            aiqubit_faucet::mint(&mut treasury_cap, 100, &mut wallet, test_scenario::ctx(&mut scenario));
            
            aiqubit_faucet::faucet(&mut wallet, test_scenario::ctx(&mut scenario));
