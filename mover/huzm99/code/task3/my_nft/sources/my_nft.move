...
module my_nft::my_nft {
   ...
   public struct CourseInfo has key,store {
    ...
    fun init(otw: MY_NFT,ctx: &mut TxContext) {
        ...
        let values :vector<std::string::String> = vector[
            utf8(b"{name} Course Series"),
            utf8(b"{image_url}"),
            utf8(b"welcome to the {name} course series,it costs {time} hours to complete!"),
            utf8(b"huzm99"),
            utf8(b"https://github.com/huzm99/letsmove"),
            ];
            
        let publisher: sui::package::Publisher=package::claim(otw,ctx);
  
        let mut display: sui::display::Display<my_nft::my_nft::CourseInfo> =display::new_with_fields<CourseInfo>(&publisher,keys,values,ctx);
