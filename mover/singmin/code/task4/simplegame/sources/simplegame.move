module simplegame::simplegame{
    use sui::coin::{Self, Coin};
    use sui::balance::{Self, Balance};
    use sui::random::{Self, Random};
    use singmin_faucet::singmin_faucet::SINGMIN_FAUCET;
    ...
    public struct Game has key {
        id: UID,
        balance: Balance<SINGMIN_FAUCET>,
        game_name:vector<u8>
    }
    ...
    public entry fun deposit(game: &mut Game, coin: &mut Coin<SINGMIN_FAUCET>, amount: u64, ctx: &mut TxContext) {
    ...
    public entry fun Play(game: &mut Game, rnd: &Random, guess: bool, coin: &mut Coin<SINGMIN_FAUCET>, amount: u64, ctx: &mut TxContext) {
        assert!(game.balance.value() >= amount, EGameInsufficientBalance);
        assert!(coin.value() >= amount, EUserInsufficientBalance);
