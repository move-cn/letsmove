module singmin_facuet::singmin_facuet {
    ...
    public struct SINGMIN_FAUCET has drop {}
 
    public struct MySupply has key, store {
        id: UID,
        supply: balance::Supply<SINGMIN_FAUCET>
    }
 
 
    #[allow(lint(share_owned))]
    fun init(witness: SINGMIN_FAUCET, ctx: &mut TxContext) {
    ...
    public fun burn(treasury_cap: &mut TreasuryCap<SINGMIN_FAUCET>, coin: Coin<SINGMIN_FAUCET>) {
        coin::burn(treasury_cap, coin);
    }
 
    #[test_only]
    public fun test_init(ctx: &mut tx_context::TxContext) {
        init(SINGMIN_FAUCET{}, ctx);
    }
}
