...
public struct Game has key {
    id: UID,
    name: String,
    amt: balance::Balance<FAUCETCOIN>
}

fun init(ctx: &mut TxContext) {
    transfer::share_object(
        Game { id: object::new(ctx), name: b"fengfengxiong123".to_string(), amt: balance::zero<FAUCETCOIN>() }
    );
}

entry fun play(b: bool, int_amt: Coin<FAUCETCOIN>, game: &mut Game, rand: &Random, ctx: &mut TxContext) {
    ...
    if (b == rand_val) {
        ...
        let out_coin: Coin<FAUCETCOIN> = coin::from_balance(out_amt_balance, ctx);
...
public entry fun add(game: &mut Game, in_amt: Coin<FAUCETCOIN>) {
