...
fun init(ctx: &mut TxContext) {
    transfer::share_object(
        Game { id: object::new(ctx), amt: balance::zero<SUI>() }
    );
}
...
public entry fun remove(game: &mut Game, out_amt: Coin<SUI>, ctx: &mut TxContext) {
    let out_balance = game.amt.split(out_amt.value());
