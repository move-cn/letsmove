...
module benjaminhang::swap {
    ...
    use coin::my_coin::{MY_COIN as COIN_1};
    use coin::my_faucet_coin::{MY_FAUCET_COIN as COIN_2};
    ...
    // 存coin1
    public entry fun deposit_coin1(bank: &mut Bank, in: Coin<COIN_1>, _: &mut TxContext) {
    ...
    // 存coin2
    public entry fun deposit_coin2(bank: &mut Bank, in: Coin<COIN_2>, _: &mut TxContext) {
        deposit(&mut bank.coin2, in);
    }

    // 取coin2
    public entry fun withdraw_coin2(_: &AdminCap, bank: &mut Bank, amt: u64, ctx: &mut TxContext) {
        withdraw(&mut bank.coin2, amt, ctx);
    }

    // swap coin1 to coin2
    public entry fun swap_1_2(bank: &mut Bank, in: Coin<COIN_1>, ctx: &mut TxContext) {
    ...
    // swap coin2 to coin1
    public entry fun swap_2_1(bank: &mut Bank, in: Coin<COIN_2>, ctx: &mut TxContext) {
        let in_value = coin::value(&in);
