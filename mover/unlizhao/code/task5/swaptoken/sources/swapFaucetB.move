module swaptoken::swapFaucetB{
    use sui::coin::{Self, TreasuryCap};
    use sui::tx_context::TxContext;

    public struct SWAPFAUCETB has drop{}

    public struct TreasuryCaoHolder has key, store{
        id: UID,
        treasury_cap: TreasuryCap<SWAPFAUCETB>,
    }

    // otw = One-time witness,确保只能在init初始化调用一次，同时1结构体只有drop能力
    fun init(otw:SWAPFAUCETB, ctx: &mut TxContext){
        let (treasury_cap, metadata) = coin::create_currency(
            otw,
            4,
            b"unlizhao_faucetB",
            b"UNLIZHAO_FAUCET_COINB",
            b"UNLIZHAO_FAUCET_COINAB",
            option::none(),
            ctx
        );
