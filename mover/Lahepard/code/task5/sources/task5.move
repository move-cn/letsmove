module task5::task5_swap {
    use sui::balance::{Self, Balance};
    use sui::coin::{Self, Coin};
    use sui::tx_context::sender;
    use task2::lahepard_faucet_coin::{LAHEPARD_FAUCET_COIN};
    use task2::lahepard_coin::{LAHEPARD_COIN};

    public struct Bank has key {
        id: UID,
        USD: Balance<LAHEPARD_COIN>,
        RMB: Balance<LAHEPARD_FAUCET_COIN>,
    ...
    fun init(ctx: &mut TxContext) {
        let bank = Bank {
            id: object::new(ctx),
            USD: balance::zero<LAHEPARD_COIN>(),
            RMB: balance::zero<LAHEPARD_FAUCET_COIN>(),
            rate: 7,
        };
    ...
    public entry fun deposit_RMB(bank:&mut Bank,rmb:Coin<LAHEPARD_FAUCET_COIN>,_:&mut TxContext){
        balance::join(&mut bank.RMB,coin::into_balance(rmb));
    }

    public entry fun deposit_USD(bank:&mut Bank,usd:Coin<LAHEPARD_COIN>,_:&mut TxContext){
        balance::join(&mut bank.USD,coin::into_balance(usd));
    }

    public entry fun swap_usd2rmb(bank: &mut Bank, usd: Coin<LAHEPARD_COIN>, ctx: &mut TxContext) {
    ...
    public entry fun swap_rmb2usd(bank: &mut Bank, rmb: Coin<LAHEPARD_FAUCET_COIN>, ctx: &mut TxContext) {
