/// Module: m4sk93_swap
module m4sk93_swap::m4sk93_swap {
    ...
    use m4sk93_coin::m4sk93_coin::{Self, M4SK93_COIN};
    
    /// 银行提供换汇服务
    public struct Bank has key {
        id: UID,
        USD: Balance<M4SK93_COIN>,
        RMB: Balance<M4SK93_FAUCET_COIN>,
    ...
    fun init(ctx: &mut TxContext) {
        let bank = Bank {
            id: object::new(ctx),
            USD: balance::zero<M4SK93_COIN>(),
            RMB: balance::zero<M4SK93_FAUCET_COIN>(),
            rate: 7,
        };
    ...
    public entry fun deposit_RMB(bank:&mut Bank,rmb:Coin<M4SK93_FAUCET_COIN>,_:&mut TxContext){
        balance::join(&mut bank.RMB,coin::into_balance(rmb));
    }

    public entry fun deposit_USD(bank:&mut Bank,usd:Coin<M4SK93_COIN>,_:&mut TxContext){
        balance::join(&mut bank.USD,coin::into_balance(usd));
    }

    public entry fun swap_usd2rmb(bank: &mut Bank, usd: Coin<M4SK93_COIN>, ctx: &mut TxContext) {
    ...
    public entry fun swap_rmb2usd(bank: &mut Bank, rmb: Coin<M4SK93_FAUCET_COIN>, ctx: &mut TxContext) {
