module game::sabrinon_game {
    use std::string::{Self, String};
    use sui::event;
    use sui::balance::{Self, Balance};
    use sui::coin::{Self, Coin, TreasuryCap};
    use sui::tx_context::sender;
    use faucet::sabrinon_faucet_coin::{Self, SABRINON_FAUCET_COIN};
    use std::hash;
    ...
    public struct Game has key {
        id: UID,
        pool: Balance<SABRINON_FAUCET_COIN>,
        ticket: u64,
        reward: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            pool: balance::zero<SABRINON_FAUCET_COIN>(),
            ticket: 1_000,
            reward: 2_000,
        };
    ...
    public entry fun play(game: &mut Game, input: Coin<SABRINON_FAUCET_COIN>, ctx: &mut TxContext) {
    ...
    public entry fun deposit(game: &mut Game, input: Coin<SABRINON_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        let input_value = coin::value(&input);
    ...
    public entry fun get_faucet_coin(treasury_cap: &mut TreasuryCap<SABRINON_FAUCET_COIN>, amount: u64, ctx: &mut TxContext) {
        sabrinon_faucet_coin::mint(treasury_cap, amount, sender(ctx), ctx);
    }
