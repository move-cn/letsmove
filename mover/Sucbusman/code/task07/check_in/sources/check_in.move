module check_in::check_in {
    ...
    entry fun get_flag(
		...
		//test insert
		event::emit(Log{bcs_flag: bcs_flag, sha: sha3_256(bcs_flag)});
		//
...
#[test]
	fun test(){
		let flags = string(b"bFen>krLm(}E");
		//let flags = string(b"LetsMoveCTF");
		let github_id = string(b"Sucbusman");
		let mut flagb = bcs::to_bytes(&flags);
        vector::append<u8>(&mut flagb, *github_id.as_bytes());
		let sha:vector<u8> = sha3_256(flagb);
		print(&flagb);
		print(&sha);
	}
}
